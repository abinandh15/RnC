...
export class FallSystem implements ISystem {
  update(dt: number) {
    for (const drop of drops.entities) {
      ...
      if (position.y < 0) {
        // position.x = Math.random() * 8 + 11
        position.x = Math.random() * (40 - 11) + 11
        position.y = 10
        // position.z = Math.random() * 8 + 4
        position.z = Math.random() * (62 - 1) + 1
      }
    }
  }
}
...
// create drop entity
function spawnRain() {
  const drop = new Entity('drop')
  drop.addComponent(new IsPrecip(PrecipType.drop))
  // const pos = new Vector3(Math.random() * 8 + 11, 10, Math.random() * 8 + 4)
  const pos = new Vector3(Math.random() * (40 - 11) + 11, 10, Math.random() * (62 - 1) + 1)
  drop.addComponent(
    new Transform({
      position: pos,
  ...
  dropShape.withCollisions = false
